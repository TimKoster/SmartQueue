...
- You can change this with the 'sqconfig' command, for example: 'sq GEO GEO TSRC'. This will reserve two nodes for jobs with the partition 'GEO' and one with the partition 'TSRC'. The other 5 runners can pick any job from the queue, you do not need to specify 'ALL'. Note that 'GEO' jobs can also run in 'ALL' partitions if they 'GEO' runners are full and it happens to be next. If there are no 'GEO' or 'TSRC' jobs, those three runners will remain empty.
- Using 'sq GEO-' will try to reserve 1 runner for 'GEO' partition jobs, but if there are none it will pull in any job.
...
- so you do not need to call this. However, jobs cancelled by time or not added by sqbatch will NOT
...
- A copy of the slurm input script is submitted (affixed with '_') with the instruction to call the queue_controller.py, which is how SmartQueue knows when to pull in new jobs. This means that cancelled jobs will not run this update, nor will jobs not submitted with sqbatch.
